...
Write-Host "  2. Telegram: talk to @BotFather -> /newbot -> get the bot token;"
Write-Host "     get your numeric user id from @userinfobot"
Write-Host "  3. Edit .env and fill in:"
Write-Host "       FIREFLY_PAT, ANTHROPIC_API_KEY,"
Write-Host "       TELEGRAM_BOT_TOKEN, TELEGRAM_ALLOWED_USER_IDS, TELEGRAM_ALERT_CHAT_ID"
Write-Host "  4. Apply the new .env:   docker compose up -d --force-recreate api worker beat bot"
